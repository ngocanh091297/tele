...
let topUser = function () {
	TaiXiu_User.find({ 'totall': { $gt: 0 } }, 'totall uid', { sort: { totall: -1 }, limit: 10 }, function (err, results) {
		if (err) {
			console.error('Lỗi truy vấn TaiXiu_User:', err);
			return;
		}
		if (results && Array.isArray(results)) {  // Kiểm tra kết quả có phải là mảng
				...
				return new Promise(function (resolve, reject) {
					UserInfo.findOne({ 'id': obj.uid }, function (error, result2) {
						if (error) {
							...
							resolve({ name: '' });  // Trả về name rỗng nếu có lỗi
						} else {
			...
			console.log('Không có kết quả hoặc kết quả không phải là mảng.');
		}
	});
};
